...
namespace LSIS {
	...
	void BVH::SetGeometryBuffers(const TypedBuffer<SHARED::Vertex>& vertices, const TypedBuffer<SHARED::Face>& faces)
	{
		...
		m_kernel.setArg(1, faces.GetBuffer());
		m_kernel.setArg(2, vertices.GetBuffer());
		m_kernel.setArg(5, sizeof(uint32_t), &num_faces);
		m_kernel.setArg(6, sizeof(uint32_t), &num_vertices);
	}

	void BVH::SetBVHBuffer(const TypedBuffer<SHARED::Node>& nodes)
	{
	...
	void BVH::Trace(const TypedBuffer<SHARED::Ray>& rays, const TypedBuffer<SHARED::Intersection>& intersections)
	{
		...
		m_kernel.setArg(3, rays.GetBuffer());
		m_kernel.setArg(7, sizeof(uint32_t), &num_rays);
		m_kernel.setArg(8, intersections.GetBuffer());
